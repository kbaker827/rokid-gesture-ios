...
final class GlassesServer: ObservableObject {
    ...
    func broadcastMenu(_ text: String) {
        ...
        RokidMobileSDK.vui.sendMessage(topic: "menu", text: String(describing: text), to: dev)
    }

    func broadcastGesture(_ emoji: String, gestureName: String, actionName: String) {
        guard let dev = activeDevice else { return }
        RokidMobileSDK.vui.sendMessage(topic: "gesture", text: String(describing: emoji), to: dev)
    }

    func broadcastSelect(_ itemTitle: String) {
        guard let dev = activeDevice else { return }
        RokidMobileSDK.vui.sendMessage(topic: "select", text: String(describing: itemTitle), to: dev)
    }
